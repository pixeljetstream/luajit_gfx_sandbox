...
#if defined(SHADERC_DLL) && defined(_SHADERC_BUILD_DLL)
 #error "Can only be either using DLL  (SHADERC_DLL) or building DLL (_SHADERC_BUILD_DLL)"
#endif

/* SHADERC is used to declare public API functions for export
 * from the DLL / shared library / dynamic library.
 */
#if defined(_WIN32) && defined(_SHADERC_BUILD_DLL)
 /* We are building SHADERC as a Win32 DLL */
 #define SHADERCAPI __declspec(dllexport)
#elif defined(_WIN32) && defined(SHADERC_DLL)
 /* We are calling SHADERC as a Win32 DLL */
 #define SHADERCAPI __declspec(dllimport)
#elif defined(__GNUC__) && defined(_SHADERC_BUILD_DLL)
 /* We are building SHADERC as a shared / dynamic library */
 #define SHADERCAPI __attribute__((visibility("default")))
#else
 /* We are building or calling SHADERC as a static library */
 #define SHADERCAPI
#endif
...
// argument.
SHADERCAPI shaderc_compiler_t shaderc_compiler_initialize(void);
...
SHADERCAPI void shaderc_compiler_release(shaderc_compiler_t);
...
SHADERCAPI shaderc_compile_options_t shaderc_compile_options_initialize(void);

// Returns a copy of the given shaderc_compile_options_t.
// If NULL is passed as the parameter the call is the same as
// shaderc_compile_options_init.
SHADERCAPI shaderc_compile_options_t shaderc_compile_options_clone(
...
SHADERCAPI void shaderc_compile_options_release(shaderc_compile_options_t options);
...
SHADERCAPI void shaderc_compile_options_add_macro_definition(
    shaderc_compile_options_t options, const char* name, size_t name_length,
    const char* value, size_t value_length);

// Sets the source language.  The default is GLSL.
SHADERCAPI void shaderc_compile_options_set_source_language(
    shaderc_compile_options_t options, shaderc_source_language lang);

// Sets the compiler mode to generate debug information in the output.
SHADERCAPI void shaderc_compile_options_set_generate_debug_info(
    shaderc_compile_options_t options);

// Sets the compiler optimization level to the given level. Only the last one
// takes effect if multiple calls of this function exist.
SHADERCAPI void shaderc_compile_options_set_optimization_level(
...
SHADERCAPI void shaderc_compile_options_set_forced_version_profile(
...
SHADERCAPI void shaderc_compile_options_set_include_callbacks(
...
// as error messages.
SHADERCAPI void shaderc_compile_options_set_suppress_warnings(
    shaderc_compile_options_t options);

// Sets the target shader environment, affecting which warnings or errors will
// be issued.  The version will be for distinguishing between different versions
// of the target environment.  "0" is the only supported version at this point
SHADERCAPI void shaderc_compile_options_set_target_env(shaderc_compile_options_t options,
...
SHADERCAPI void shaderc_compile_options_set_warnings_as_errors(
    shaderc_compile_options_t options);

// Sets a resource limit.
SHADERCAPI void shaderc_compile_options_set_limit(
...
SHADERCAPI shaderc_compilation_result_t shaderc_compile_into_spv(
...
SHADERCAPI shaderc_compilation_result_t shaderc_compile_into_spv_assembly(
    const shaderc_compiler_t compiler, const char* source_text,
    size_t source_text_size, shaderc_shader_kind shader_kind,
    const char* input_file_name, const char* entry_point_name,
    const shaderc_compile_options_t additional_options);
...
SHADERCAPI shaderc_compilation_result_t shaderc_compile_into_preprocessed_text(
    const shaderc_compiler_t compiler, const char* source_text,
    size_t source_text_size, shaderc_shader_kind shader_kind,
    const char* input_file_name, const char* entry_point_name,
    const shaderc_compile_options_t additional_options);
...
// returned.
SHADERCAPI shaderc_compilation_result_t shaderc_assemble_into_spv(
...
SHADERCAPI void shaderc_result_release(shaderc_compilation_result_t result);

// Returns the number of bytes of the compilation output data in a result
// object.
SHADERCAPI size_t shaderc_result_get_length(const shaderc_compilation_result_t result);

// Returns the number of warnings generated during the compilation.
SHADERCAPI size_t shaderc_result_get_num_warnings(
    const shaderc_compilation_result_t result);

// Returns the number of errors generated during the compilation.
SHADERCAPI size_t shaderc_result_get_num_errors(const shaderc_compilation_result_t result);

// Returns the compilation status, indicating whether the compilation succeeded,
// or failed due to some reasons, like invalid shader stage or compilation
// errors.
SHADERCAPI shaderc_compilation_status shaderc_result_get_compilation_status(
...
SHADERCAPI const char* shaderc_result_get_bytes(const shaderc_compilation_result_t result);

// Returns a null-terminated string that contains any error messages generated
// during the compilation.
SHADERCAPI const char* shaderc_result_get_error_message(
    const shaderc_compilation_result_t result);

// Provides the version & revision of the SPIR-V which will be produced
SHADERCAPI void shaderc_get_spv_version(unsigned int* version, unsigned int* revision);
...
SHADERCAPI bool shaderc_parse_version_profile(const char* str, int* version,
